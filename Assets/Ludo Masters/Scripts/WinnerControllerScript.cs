...
public class WinnerControllerScript : MonoBehaviour
{
    ...
    public void SendMessageEvent(string index)
    {
        ...
        if (!GameManager.Instance.offlineMode)
            PhotonNetwork.RaiseEvent(193, index, new Photon.Realtime.RaiseEventOptions(), new ExitGames.Client.Photon.SendOptions() { Reliability = true });
    ...
    public void loadMenuScene()
    {
        ...
        //PhotonNetwork.BackgroundTimeout = StaticStrings.photonDisconnectTimeoutLong; ;
        if (!GameManager.Instance.offlineMode)
            PhotonNetwork.RaiseEvent(194, 1, new Photon.Realtime.RaiseEventOptions(), new ExitGames.Client.Photon.SendOptions() { Reliability = true });
        removeOnEventCall();
    ...
    public void sendRematchRequest()
    {
        if (!rematchRequest)
        {
            ...
            if (!GameManager.Instance.offlineMode)
                PhotonNetwork.RaiseEvent(195, 1, new Photon.Realtime.RaiseEventOptions(), new ExitGames.Client.Photon.SendOptions() { Reliability = true });
            myMessageBubble.SetActive(true);
    ...
    public void OnEvent(EventData eventData)
    {
        ...
        else if (eventData.Code == 193)
        {
            string index = (string)eventData.CustomData;
