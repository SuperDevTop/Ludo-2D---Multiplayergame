...
public class GameGUIController : MonoBehaviourPunCallbacks
{
    ...
    public GameObject showParticle;
    ...
    void Start()
    {
        ...
        PhotonNetwork.RaiseEvent((int)EnumPhoton.ReadyToPlay, 0, new Photon.Realtime.RaiseEventOptions(), new ExitGames.Client.Photon.SendOptions() { Reliability = true });
    ...
    public void AddFriendButtonClick()
    {
        if (!CurrentPlayerID.Contains("_BOT"))
        {
            ...
            {
                PhotonNetwork.RaiseEvent((int)EnumPhoton.AddFriend, PhotonNetwork.LocalPlayer.NickName + ";" + GameManager.Instance.nameMy + ";" + CurrentPlayerID, new Photon.Realtime.RaiseEventOptions(), new ExitGames.Client.Photon.SendOptions() { Reliability = true });
    ...
    private void SetFinishGame(string id, bool me)
    {
        if (!me || !iFinished)
        {
            ...
            //We change here (me) or (!GameManager.Instance.currentPlayer.isBot)
            if (me)
            {
                ...
                {
                    PhotonNetwork.RaiseEvent((int)EnumPhoton.FinishedGame, PhotonNetwork.LocalPlayer.NickName, new Photon.Realtime.RaiseEventOptions(), new ExitGames.Client.Photon.SendOptions() { Reliability = true });
                ...
                if (position == 1)
                {
                    Common.ScreenLog.ForceLog("Finish game API hit");
                    GenieAPIClass.Instance.FinishTheGame(Ludo.IN.GameManager.Instance.Genie_user_id);
    ...
    public void SendFinishTurn()
    {
        if (!FinishWindowActive && ActivePlayersInRoom > 1)
        {
            ...
            {
                PhotonNetwork.RaiseEvent((int)EnumPhoton.NextPlayerTurn, myIndex, new Photon.Realtime.RaiseEventOptions(), new ExitGames.Client.Photon.SendOptions() { Reliability = true });
    ...
    {
        Common.ScreenLog.ForceLog("Event Received: " + data.Code);
        Debug.Log("received event: " + data.Code);
        if (data.Code == (int)EnumPhoton.NextPlayerTurn)
        {
            Common.ScreenLog.ForceLog("Next player turn");
            if (playerObjects[(int)data.CustomData].AvatarObject.GetComponent<PlayerAvatarController>().Active &&
                currentPlayerIndex == (int)data.CustomData)
            {
                if (!FinishWindowActive)
                {
                    setCurrentPlayerIndex((int)data.CustomData);

                    SetTurn();
                }
            }
        }
        else if (data.Code == (int)EnumPhoton.SendChatMessage)
        {
            string[] message = ((string)data.CustomData).Split(';');
        ...
        else if (data.Code == (int)EnumPhoton.SendChatEmojiMessage)
        {
            string[] message = ((string)data.CustomData).Split(';');
            Debug.Log("Received message " + message[0] + " from " + message[1]);
            for (int i = 0; i < playerObjects.Count; i++)
        ...
        else if (data.Code == (int)EnumPhoton.AddFriend)
        {
            if (PlayerPrefs.GetInt(StaticStrings.FriendsRequestesKey, 0) == 0)
            {
                string[] _data = ((string)data.CustomData).Split(';');
                if (PhotonNetwork.LocalPlayer.NickName.Equals(data[2]))
                    invitiationDialog.GetComponent<PhotonChatListener2>().showInvitationDialog(_data[0], _data[1], null);
            }
            else
            {
        ...
        else if (data.Code == (int)EnumPhoton.FinishedGame)
        {
    ...
    private void SetTurn()
    {
        ...
        Common.ScreenLog.ForceLog("Set turn");
        for (int i = 0; i < playerObjects.Count; i++)
        {
        ...
        {
            Common.ScreenLog.ForceLog("My turn");
            SetMyTurn();
        }
        else if (playerObjects[currentPlayerIndex].isBot)
        {
            Common.ScreenLog.ForceLog("Bot Turn");
            BotTurn();
        }
        else
        {
            Common.ScreenLog.ForceLog("Opponent turn");
            SetOpponentTurn();
        }
    }
